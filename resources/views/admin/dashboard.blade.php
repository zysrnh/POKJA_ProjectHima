...
<head>
    ...
    <title>Dashboard Admin - POKJA HIMA IF</title>
    <!-- Tailwind CSS CDN -->
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <h1 class="text-base sm:text-lg font-bold text-gray-900 uppercase tracking-wide">POKJA HIMA IF</h1>
                </div>
                ...
                <div class="flex items-center gap-2 sm:gap-4">
                    ...
                    <!-- User & Logout -->
                    <span class="text-xs text-gray-600 hidden md:inline font-medium">Halo, {{ Auth::user()->name ?? 'Admin' }}</span>
                    
                    <form action="{{ route('admin.logout') }}" method="POST" class="inline">
                        @csrf
                        <button 
                            type="submit" 
                            class="px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-semibold uppercase tracking-wider transition cursor-pointer"
                        >
                            Keluar
                        </button>
                    </form>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6">
        ...
        <div class="bg-white border border-gray-300 shadow-sm overflow-hidden">
            <div class="px-4 py-3 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
                <h2 class="text-xs font-bold uppercase tracking-wider text-gray-700">Daftar Mahasiswa Pendaftar POKJA</h2>
                ...
                <table class="w-full text-left border-collapse text-xs sm:text-sm">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($pendaftarans as $index => $item)
                            @php
                                // Bersihkan nomor telepon untuk format link WhatsApp (misal 0812 -> 62812)
